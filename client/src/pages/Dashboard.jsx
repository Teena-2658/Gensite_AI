    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans">
      ...
      <nav className="bg-white/90 backdrop-blur-md border-b sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => { localStorage.removeItem("user"); navigate("/"); }} className="text-red-500 font-medium text-sm px-2 hover:underline flex items-center gap-1">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-10">
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          ...
          {/* CREDITS BUY BOX - NOW FUNCTIONAL */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {websites.length > 0 ? (
            websites.map((site, index) => (
              <div key={site._id} className="group bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_20px_50px_rgba(59,130,246,0.12)] transition-all duration-500 flex flex-col h-full">
                <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
                  <img src={`https://images.unsplash.com/photo-1460925895917?q=80&w=500&auto=format&fit=crop&sig=${index}`} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700" alt="Web Preview" />
                  <div className="absolute top-4 left-4 z-10">
                    <div className="bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-2xl text-[10px] text-slate-800 font-bold border border-white/50 shadow-sm flex items-center gap-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${site.deployed ? 'bg-green-500' : 'bg-amber-500'}`} />
                ...
                <div className="p-6 flex flex-col flex-1">
                  <h4 className="text-lg font-black text-slate-800 mb-2 line-clamp-2 leading-tight min-h-[3rem]">{site.title || "Untitled Project"}</h4>
                  <div className="flex items-center gap-2 mb-6">
                    <Monitor size={14} className="text-slate-400" />
                    <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Updated {new Date(site.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="mt-auto flex gap-2">
                    <button onClick={() => navigate(`/preview/${site._id}`)} className="flex-1 bg-slate-900 text-white py-3 rounded-2xl text-[11px] font-black hover:bg-blue-600 transition-all flex items-center justify-center gap-2">
                      EDIT SITE <ArrowRight size={14}/>
                    </button>
                    {site.deployedUrl && (
                      <button onClick={() => window.open(site.deployedUrl, '_blank')} className="w-12 h-12 flex items-center justify-center bg-blue-50 text-blue-600 rounded-2xl border border-blue-100 hover:bg-blue-600 hover:text-white transition-all">
                        <Globe size={18} />
                      </button>
                    )}
                  </div>
                  <button onClick={() => deleteWebsite(site._id)} className="mt-4 text-[10px] font-bold text-slate-300 hover:text-red-500 transition-colors uppercase tracking-[0.1em] self-center">Archive Project</button>
                </div>
              </div>
            ))
          ) : (
            <div className="col-span-full py-24 text-center bg-white rounded-[3.5rem] border-2 border-dashed border-slate-100 shadow-inner">
              ...
              <button onClick={() => textareaRef.current?.focus()} className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-bold text-sm hover:bg-blue-700 transition-all">Create Your First Site</button>
            </div>
          )}
        </div>
      </main>
